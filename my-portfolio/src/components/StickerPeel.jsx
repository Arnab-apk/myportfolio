...
const StickerPeel = ({
  ...
  return (
    <div ref={dragTargetRef} className={`relative cursor-grab active:cursor-grabbing transform-gpu ${className}`} style={cssVars}>
      <style>{`
      ...
      <div ref={containerRef} className="sticker-container relative select-none" style={{ transform: `rotate(${peelDirection}deg)`, transformOrigin: 'center' }}>
        <div className="sticker-main" style={stickerMainStyle}>
          <div style={{ filter: 'url(#pointLight)' }}>
            <img src={imageSrc} alt="sticker" className="block" style={imageStyle} draggable="false" onContextMenu={e => e.preventDefault()} />
          </div>
        </div>
        <div className="absolute top-4 left-2 w-full h-full opacity-40" style={{ filter: 'brightness(0) blur(8px)' }}>
          <div className="sticker-flap" style={flapStyle}>
            <img src={imageSrc} alt="shadow" className="block" style={shadowImageStyle} draggable="false" onContextMenu={e => e.preventDefault()} />
          </div>
        </div>
        <div className="sticker-flap absolute w-full h-full left-0" style={flapStyle}>
          <div style={{ filter: 'url(#pointLightFlipped)' }}>
            <img src={imageSrc} alt="gloss" className="block" style={shadowImageStyle} draggable="false" onContextMenu={e => e.preventDefault()} />
          </div>
        </div>
      </div>
    </div>
