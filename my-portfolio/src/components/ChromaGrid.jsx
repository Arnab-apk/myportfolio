...
const ChromaGrid = ({ items, className = '', radius = 350, damping = 0.45, fadeOut = 0.6, ease = 'power3.out' }) => {
    ...
    >
      {data.map((c, i) => (
        <article
          ...
          className="group relative flex flex-col w-[300px] h-[400px] rounded-[24px] overflow-hidden border-2 transition-all duration-500 cursor-pointer hover:scale-[1.02] hover:shadow-2xl"
          style={{
        ...
        >
          ...
          {/* Card footer */}
          <footer className="relative z-10 p-4 text-white font-sans backdrop-blur-sm bg-black/20">
            <div className="flex items-center mb-2">
              <h3 className="m-0 text-lg font-bold">{c.title}</h3>
            </div>
            <p className="m-0 text-sm opacity-90 leading-relaxed">{c.subtitle}</p>
            {c.location && <span className="text-xs opacity-75 mt-1 block">{c.location}</span>}
          </footer>
        </article>
      ))}
      ...
      <div
        ref={fadeRef}
        className="absolute inset-0 pointer-events-none transition-opacity duration-300 z-40 hidden md:block"
        style={{
          backdropFilter: 'grayscale(1) brightness(0.75)',
          WebkitBackdropFilter: 'grayscale(1) brightness(0.75)',
          background: 'rgba(0,0,0,0.001)',
