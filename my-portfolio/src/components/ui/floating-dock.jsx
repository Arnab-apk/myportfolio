...
const FloatingDockDesktop = ({ items, className }) => {
  ...
  return (
    <motion.div
      ...
      className={cn(
        "mx-auto hidden md:flex h-12 gap-3 items-end rounded-2xl bg-slate-900/80 backdrop-blur-md border border-slate-700/50 px-3 pb-2",
        className
      )}
    >
      {items.map((item) => (
  ...
  return (
    <a href={href} onClick={handleClick}>
      <motion.div
        ...
        className="aspect-square rounded-full bg-slate-800 border border-slate-700 flex items-center justify-center relative"
      >
        <AnimatePresence>
          {hovered && (
            <motion.div
              ...
              className="px-2 py-0.5 whitespace-pre rounded-md bg-slate-800 border border-slate-700 text-white absolute left-1/2 -translate-x-1/2 -top-8 w-fit text-xs"
            >
              {title}
            </motion.div>
          )}
